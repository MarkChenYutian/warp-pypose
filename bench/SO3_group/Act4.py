...
from pypose_warp.ltype.SO3_group import SO3_Act4
...
def bench_forward(num: int, device: T.Literal["cpu", "cuda"], dtype: torch.dtype, kind: T.Literal["n-n", "1-n", "n-1"]):
    ...
    points   = torch.randn((num_point, 4), device=device, dtype=dtype)  # 4D points
...
def bench_backward(num: int, device: T.Literal["cpu", "cuda"], dtype: torch.dtype, kind: T.Literal["n-n", "1-n", "n-1"]):
    ...
    # Warp backward (using SO3_Act4.apply directly)
    def wp_backward():
        poses = pp.randn_SO3(num_pose, device=device, dtype=dtype, requires_grad=True)
        points = torch.randn((num_point, 4), device=device, dtype=dtype, requires_grad=True)  # 4D points
        result = SO3_Act4.apply(poses, points)
        result.sum().backward()
...
def main():
    parser = argparse.ArgumentParser(description="Benchmark SO3 Act4 forward/backward (4D homogeneous points)")
    ...
    print(f"Benchmarking SO3.Act4 {args.mode} | device={args.device} | dtype={args.dtype} | size={args.size} | kind={args.kind}")
    print("-" * 80)
