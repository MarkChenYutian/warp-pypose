"""
Benchmark SO3.matrix() operation: quaternion to 3x3 rotation matrix conversion.

Compares PyPose's implementation (using Act on identity matrix) with Warp's
direct quat_to_matrix kernel.
"""
import argparse
import torch
import typing as T
...
from pypose_warp.ltype.SO3_group import SO3_Mat
...
def bench_forward(num: int, device: T.Literal["cpu", "cuda"], dtype: torch.dtype):
    ...
    pp_timer = Timer(stmt="pose.matrix()", globals=dict(pose=pp_poses))
    wp_timer = Timer(stmt="pose.matrix()", globals=dict(pose=wp_poses))
...
def bench_backward(num: int, device: T.Literal["cpu", "cuda"], dtype: torch.dtype):
    ...
    # Warp backward (using SO3_Mat.apply directly)
    def wp_backward():
        poses = pp.randn_SO3(num, device=device, dtype=dtype, requires_grad=True)
        result = SO3_Mat.apply(poses)
        result.sum().backward()
...
def main():
    parser = argparse.ArgumentParser(description="Benchmark SO3 matrix() forward/backward")
    ...
    print(f"Benchmarking SO3.matrix {args.mode} | device={args.device} | dtype={args.dtype} | size={args.size}")
    print("-" * 80)
