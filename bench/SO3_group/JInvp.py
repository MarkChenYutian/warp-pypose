"""
Benchmark for SO3_Jinvp: Left Jacobian Inverse Product

Jinvp computes: Jl_inv(Log(X)) @ p
where:
  - X is an SO3 rotation (quaternion)
  - p is a tangent vector (so3 / axis-angle)
  - Jl_inv is the inverse of the left Jacobian of SO3
  - Log maps SO3 to so3 (Lie algebra)
"""
import argparse
import torch
import typing as T
import pypose as pp
import warp as wp

wp.init()

from pypose.lietensor.operation import so3_Jl_inv, SO3_Log
from pypose_warp import to_warp_backend
from pypose_warp.ltype.SO3_group import SO3_Jinvp
from torch.utils.benchmark import Timer


def pypose_jinvp(X: pp.LieTensor, p: torch.Tensor) -> torch.Tensor:
    """Reference PyPose implementation of Jinvp."""
    return (so3_Jl_inv(SO3_Log.apply(X.tensor())) @ p.unsqueeze(-1)).squeeze(-1)


def bench_forward(num: int, device: T.Literal["cpu", "cuda"], dtype: torch.dtype, kind: T.Literal["n-n", "1-n", "n-1"]):
    """Benchmark forward pass of Jinvp."""
    match kind:
        case "n-n": num_pose, num_vec = num, num
        case "1-n": num_pose, num_vec = 1  , num
        case "n-1": num_pose, num_vec = num, 1
    
    pp_poses = pp.randn_SO3(num_pose, device=device, dtype=dtype)
    wp_poses = to_warp_backend(pp_poses)
    tangent  = torch.randn((num_vec, 3), device=device, dtype=dtype)
    
    # PyPose: use the reference implementation
    pp_timer = Timer(
        stmt="pypose_jinvp(pose, tangent)",
        globals=dict(pypose_jinvp=pypose_jinvp, pose=pp_poses, tangent=tangent)
    )
    
    # Warp: use the Jinvp method on warp backend poses
    wp_timer = Timer(
        stmt="pose.Jinvp(tangent)",
        globals=dict(pose=wp_poses, tangent=tangent)
    )
    
    pp_bench = pp_timer.adaptive_autorange()
    wp_bench = wp_timer.adaptive_autorange()
    return pp_bench, wp_bench


def bench_backward(num: int, device: T.Literal["cpu", "cuda"], dtype: torch.dtype, kind: T.Literal["n-n", "1-n", "n-1"]):
    """Benchmark backward pass of Jinvp."""
    match kind:
        case "n-n": num_pose, num_vec = num, num
        case "1-n": num_pose, num_vec = 1  , num
        case "n-1": num_pose, num_vec = num, 1
    
    # PyPose backward
    def pp_backward():
        poses = pp.randn_SO3(num_pose, device=device, dtype=dtype, requires_grad=True)
        tangent = torch.randn((num_vec, 3), device=device, dtype=dtype, requires_grad=True)
        result = pypose_jinvp(poses, tangent)
        result.sum().backward()
    
    # Warp backward (using SO3_Jinvp.apply directly)
    def wp_backward():
        poses = pp.randn_SO3(num_pose, device=device, dtype=dtype, requires_grad=True)
        tangent = torch.randn((num_vec, 3), device=device, dtype=dtype, requires_grad=True)
        result = SO3_Jinvp.apply(poses, tangent)
        result.tensor().sum().backward()
    
    pp_timer = Timer(stmt="pp_backward()", globals=dict(pp_backward=pp_backward))
    wp_timer = Timer(stmt="wp_backward()", globals=dict(wp_backward=wp_backward))
    
    pp_bench = pp_timer.adaptive_autorange()
    wp_bench = wp_timer.adaptive_autorange()
    return pp_bench, wp_bench


DTYPE_MAP = {
    "fp16": torch.float16,
    "fp32": torch.float32,
    "fp64": torch.float64,
}


def main():
    parser = argparse.ArgumentParser(description="Benchmark SO3 Jinvp forward/backward")
    parser.add_argument("--mode", choices=["fwd", "bwd"], default="fwd", help="Benchmark forward or backward pass")
    parser.add_argument("--device", choices=["cpu", "cuda"], default="cuda", help="Device to run on")
    parser.add_argument("--dtype", choices=["fp16", "fp32", "fp64"], default="fp32", help="Data type")
    parser.add_argument("--size", type=int, default=10000, help="Batch size")
    parser.add_argument("--kind", choices=["n-n", "1-n", "n-1"], default="n-n", help="Broadcasting pattern")
    args = parser.parse_args()
    
    dtype = DTYPE_MAP[args.dtype]
    
    if args.device == "cuda" and not torch.cuda.is_available():
        print("CUDA not available, falling back to CPU")
        args.device = "cpu"
    
    print(f"Benchmarking SO3.Jinvp {args.mode} | device={args.device} | dtype={args.dtype} | size={args.size} | kind={args.kind}")
    print("-" * 80)
    
    if args.mode == "fwd":
        pp_bench, wp_bench = bench_forward(args.size, args.device, dtype, args.kind)
    else:
        pp_bench, wp_bench = bench_backward(args.size, args.device, dtype, args.kind)
    
    print(f"PyPose:  {pp_bench}")
    print(f"Warp:    {wp_bench}")
    print("-" * 80)
    
    speedup = pp_bench.median / wp_bench.median
    print(f"Speedup: {speedup:.2f}x {'(Warp faster)' if speedup > 1 else '(PyPose faster)'}")


if __name__ == "__main__":
    main()

