...
from pypose_warp.ltype.SO3_group import SO3_AdjTXa
...
def bench_forward(num: int, device: T.Literal["cpu", "cuda"], dtype: torch.dtype, kind: T.Literal["n-n", "1-n", "n-1"]):
    ...
    pp_timer = Timer(stmt="pose.AdjT(a)", globals=dict(pose=pp_poses, a=pp_algebra))
    wp_timer = Timer(stmt="pose.AdjT(a)", globals=dict(pose=wp_poses, a=pp_algebra))
...
def bench_backward(num: int, device: T.Literal["cpu", "cuda"], dtype: torch.dtype, kind: T.Literal["n-n", "1-n", "n-1"]):
    ...
    def pp_backward():
        ...
        result = poses.AdjT(algebra)
        result.sum().backward()
    
    # Warp backward (using SO3_AdjTXa.apply directly)
    def wp_backward():
        poses = pp.randn_SO3(num_pose, device=device, dtype=dtype, requires_grad=True)
        algebra = pp.randn_so3(num_algebra, device=device, dtype=dtype, requires_grad=True)
        result = SO3_AdjTXa.apply(poses, algebra)
        result.sum().backward()
...
def main():
    parser = argparse.ArgumentParser(description="Benchmark SO3 AdjTXa forward/backward")
    ...
    print(f"Benchmarking SO3.AdjT (AdjTXa) {args.mode} | device={args.device} | dtype={args.dtype} | size={args.size} | kind={args.kind}")
    print("-" * 80)
