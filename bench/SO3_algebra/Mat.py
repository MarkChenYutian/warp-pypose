"""
Benchmark so3.matrix() operation: axis-angle to 3x3 rotation matrix conversion.

Compares PyPose's implementation (using Exp then Act on identity matrix) with 
Warp's optimized kernel.
"""
import argparse
import torch
import typing as T
...
from pypose_warp.ltype.SO3_algebra import so3_Mat
...
def bench_forward(num: int, device: T.Literal["cpu", "cuda"], dtype: torch.dtype):
    ...
    pp_timer = Timer(stmt="so3.matrix()", globals=dict(so3=pp_so3))
    wp_timer = Timer(stmt="so3.matrix()", globals=dict(so3=wp_so3))
