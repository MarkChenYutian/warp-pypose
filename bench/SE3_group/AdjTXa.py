...
from pypose_warp.ltype.SE3_group import SE3_AdjTXa
from torch.utils.benchmark import Timer


def bench_forward(num: int, device: T.Literal["cpu", "cuda"], dtype: torch.dtype):
    """
    Benchmark forward pass of SE3 AdjTXa (transpose adjoint).
    ...
    pp_timer = Timer(stmt="X.AdjT(a)", globals=dict(X=pp_X, a=pp_a))
    wp_timer = Timer(stmt="X.AdjT(a)", globals=dict(X=wp_X, a=pp_a))
...
def bench_backward(num: int, device: T.Literal["cpu", "cuda"], dtype: torch.dtype):
    """
    Benchmark backward pass of SE3 AdjTXa.
    ...
    # Warp backward (using SE3_AdjTXa.apply directly)
    def wp_backward():
        X = pp.randn_SE3(num, device=device, dtype=dtype, requires_grad=True)
        a = pp.randn_se3(num, device=device, dtype=dtype, requires_grad=True)
...
def main():
    parser = argparse.ArgumentParser(description="Benchmark SE3 AdjTXa forward/backward")
    ...
    print(f"Benchmarking SE3.AdjT {args.mode} | device={args.device} | dtype={args.dtype} | size={args.size}")
    print("-" * 80)
