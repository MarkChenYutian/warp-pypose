...
from pypose_warp.ltype.SE3_group import SE3_Act
...
def bench_backward(num: int, device: T.Literal["cpu", "cuda"], dtype: torch.dtype, kind: T.Literal["n-n", "1-n", "n-1"]):
    ...
    # Warp backward (using SE3_Act.apply directly)
    def wp_backward():
        poses = pp.randn_SE3(num_pose, device=device, dtype=dtype, requires_grad=True)
        points = torch.randn((num_point, 3), device=device, dtype=dtype, requires_grad=True)
        result = SE3_Act.apply(poses, points)
        result.sum().backward()
...
def main():
    parser = argparse.ArgumentParser(description="Benchmark SE3 Act forward/backward")
    ...
    print(f"Benchmarking SE3.Act {args.mode} | device={args.device} | dtype={args.dtype} | size={args.size} | kind={args.kind}")
    print("-" * 80)
