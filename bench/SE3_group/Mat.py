"""
Benchmark for SE3 Mat: convert SE3 pose to 4x4 transformation matrix.

Compares performance of:
- Warp backend: direct computation from pose components
- PyPose backend: Act on 4x4 identity matrix
"""
import argparse
import torch
import typing as T
...
from pypose_warp.ltype.SE3_group import SE3_Mat
...
def bench_backward(num: int, device: T.Literal["cpu", "cuda"], dtype: torch.dtype):
    ...
    # Warp backward (using SE3_Mat.apply directly)
    def wp_backward():
        poses = pp.randn_SE3(num, device=device, dtype=dtype, requires_grad=True)
        wp_poses = to_warp_backend(poses)
        result = SE3_Mat.apply(wp_poses)
        result.sum().backward()
...
def main():
    parser = argparse.ArgumentParser(description="Benchmark SE3 Mat forward/backward")
    ...
    print(f"Benchmarking SE3.Mat {args.mode} | device={args.device} | dtype={args.dtype} | size={args.size}")
    print("-" * 80)
