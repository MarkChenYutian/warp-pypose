"""Unit tests for SE3_Act forward and backward."""
import pytest
import torch
import pypose as pp
...
class TestSE3ActBatchDimensions:
    """Test SE3_Act_fwd with various batch dimensions."""
...
class TestSE3ActBroadcasting:
    """Test SE3_Act_fwd broadcasting behavior."""
...
class TestSE3ActPrecision:
    """Test SE3_Act_fwd precision handling."""
...
class TestSE3ActEdgeCases:
    """Test SE3_Act_fwd edge cases."""
    ...
    def test_pure_translation(self, device):
        ...
        # Pure translation: out = t + p
        expected = translation + points
        torch.testing.assert_close(result, expected)
...
class TestSE3ActErrors:
    """Test SE3_Act_fwd error handling."""
...
class TestSE3ActBwdBatchDimensions:
    """Test SE3_Act backward with various batch dimensions."""
    ...
    def test_1d_batch(self, device, dtype_bwd):
        ...
        result_ours = SE3_Act.apply(se3_ours, points_ours)
        ...
        torch.testing.assert_close(se3_ours.grad, se3_ref.grad, **get_bwd_tolerances(dtype_bwd, Operator.SE3_Act))
        torch.testing.assert_close(points_ours.grad, points_ref.grad, **get_bwd_tolerances(dtype_bwd, Operator.SE3_Act))
...
class TestSE3ActBwdBroadcasting:
    """Test SE3_Act backward with broadcasting."""
...
class TestSE3ActBwdPrecision:
    """Test SE3_Act backward precision handling."""
    ...
    def test_grad_dtype_preserved(self, device, dtype_bwd):
        ...
        result = SE3_Act.apply(se3, points)
        result.sum().backward()
...
class TestSE3ActBwdEdgeCases:
    """Test SE3_Act backward edge cases."""
    ...
    def test_identity_transform_gradient(self, device):
        ...
        # For identity transform, point gradient should be all ones (for sum loss)
    ...
    def test_large_batch_gradient(self, device):
        ...
        torch.testing.assert_close(se3_ours.grad, se3_ref.grad, **get_bwd_tolerances(dtype, Operator.SE3_Act))
        torch.testing.assert_close(points_ours.grad, points_ref.grad, **get_bwd_tolerances(dtype, Operator.SE3_Act))
...
class TestSE3ActWarpBackend:
    """Test SE3_Act through the warp_SE3Type backend."""

    def test_warp_backend_integration(self, device, dtype):
        """Test that warp_SE3Type.Act works correctly."""
    ...
    def test_warp_backend_gradient(self, device, dtype_bwd):
        """Test that warp_SE3Type.Act gradients work correctly."""
        from pypose_warp import to_warp_backend
        ...
        torch.testing.assert_close(se3_warp.grad, se3_ref.grad, **get_bwd_tolerances(dtype_bwd, Operator.SE3_Act))
        torch.testing.assert_close(points_ours.grad, points_ref.grad, **get_bwd_tolerances(dtype_bwd, Operator.SE3_Act))
